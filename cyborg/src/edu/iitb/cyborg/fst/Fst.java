...
//		}
		...
		try{
			builder = new ProcessBuilder("cmd.exe","/c","fstcompile " +
			...
			builder = new ProcessBuilder("bash","-c","fstcompile " +
					"--isymbols=fst/lexicon.syms --osymbols=fst/lexicon.syms fst/wordGraph.stxt" +
					" | fstrmepsilon | fstarcsort > fst/wordGraph_dir.fst");
			builder.redirectErrorStream(true);
			p = builder.start();	
...
//		}
			...
			try{
			    ...
			    builder = new ProcessBuilder("bash","-c","fstcompile --isymbols=fst/lexicon.syms --osymbols=fst/lexicon.syms " +
		      				"fst/lexicon.stxt | fstclosure | fstarcsort > fst/lexicon_dir.fst");
		      	builder.redirectErrorStream(true);
		      	p = builder.start();    	
			}
...
//      		}
            ...
            //composing the fst file for lexicon..
        		...
        		p = builder.start();
      		    	...
      		    	builder = new ProcessBuilder("bash","-c","fstcompose fst/lexicon_dir.fst fst/wordGraph_dir.fst | " +
            				"fstrmepsilon | fstdeterminize | fstminimize > fst/final_dir.fst");
            		builder.redirectErrorStream(true);
            		p = builder.start();	
      		    }
                ...
                //print the final  fst file
             		...
             		p = builder.start();
             			...
             			builder = new ProcessBuilder("bash","-c","fstprint --isymbols=fst/lexicon.syms " +
                         		"--osymbols=fst/lexicon.syms fst/final_dir.fst fst/final_dir.txt");
                 		builder.redirectErrorStream(true);
                 		p = builder.start();	
             		}
